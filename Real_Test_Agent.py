...
class compute_state_input:
    ...
    def compute(self,state,action):
        ...
        self.intergal_error += error*0.001
...
Agent = DDPGAgent(state_dim=len(state_dim_list),action_dim=1,min_action=0,max_action=10,replay_buffer='per')
...
action = 0
try:
    data_action,data_state,data_per_error,datal_intergla = [],[],[],[]
    data_time = []
    while True :
        ...
        data_per_error.append(per_error)
        datal_intergla.append(intergla)
finally:
    logger.add_data_log(["data_action","data_state","data_per_error","datal_intergla"],[data_action,data_state,data_per_error,datal_intergla])
    logger.save_to_csv(file_name="test_aget_real_work"+f"_{number_file}",folder_name=Foldor+r'/data_log_test')
